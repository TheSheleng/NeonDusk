...
#include "Characters/NeonDuskCharacter.h"
#include "Components/InputComponent.h"
#include "Engine/BlueprintGeneratedClass.h"
#include "Engine/LocalPlayer.h"
#include "DefaultMovementSet/CharacterMoverComponent.h"
#include "MoveLibrary/BasedMovementUtils.h"
#include "Characters/AbilityInputs.h"
...
ANeonDuskCharacter::ANeonDuskCharacter(const FObjectInitializer& ObjectInitializer)
...
void ANeonDuskCharacter::PostInitializeComponents()
{
...
// Called every frame
void ANeonDuskCharacter::Tick(float DeltaTime)
{
...
void ANeonDuskCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void ANeonDuskCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{	
		Input->BindAction(MoveInputAction, ETriggerEvent::Triggered, this, &ANeonDuskCharacter::OnMoveTriggered);
		Input->BindAction(MoveInputAction, ETriggerEvent::Completed, this, &ANeonDuskCharacter::OnMoveCompleted);
		Input->BindAction(LookInputAction, ETriggerEvent::Triggered, this, &ANeonDuskCharacter::OnLookTriggered);
		Input->BindAction(LookInputAction, ETriggerEvent::Completed, this, &ANeonDuskCharacter::OnLookCompleted);
		Input->BindAction(JumpInputAction, ETriggerEvent::Started, this, &ANeonDuskCharacter::OnJumpStarted);
		Input->BindAction(JumpInputAction, ETriggerEvent::Completed, this, &ANeonDuskCharacter::OnJumpReleased);
		Input->BindAction(FlyInputAction, ETriggerEvent::Triggered, this, &ANeonDuskCharacter::OnFlyTriggered);
	}
}


FVector ANeonDuskCharacter::GetNavAgentLocation() const
{
...
void ANeonDuskCharacter::UpdateNavigationRelevance()
{
	if (CharacterMotionComponent)
	{
		if (USceneComponent* UpdatedComponent = CharacterMotionComponent->GetUpdatedComponent())
...
void ANeonDuskCharacter::ProduceInput_Implementation(int32 SimTimeMs, FMoverInputCmdContext& InputCmdResult)
{
...
void ANeonDuskCharacter::OnProduceInput(float DeltaMs, FMoverInputCmdContext& OutInputCmd)
{
...
void ANeonDuskCharacter::OnMoveTriggered(const FInputActionValue& Value)
{
...
void ANeonDuskCharacter::OnMoveCompleted(const FInputActionValue& Value)
{
	CachedMoveInputIntent = FVector::ZeroVector;
}

void ANeonDuskCharacter::OnLookTriggered(const FInputActionValue& Value)
{
...
void ANeonDuskCharacter::OnLookCompleted(const FInputActionValue& Value)
{
	CachedLookInput = FRotator::ZeroRotator;
	CachedTurnInput = FRotator::ZeroRotator;
}

void ANeonDuskCharacter::OnJumpStarted(const FInputActionValue& Value)
{
	bIsJumpJustPressed = !bIsJumpPressed;
	bIsJumpPressed = true;
}

void ANeonDuskCharacter::OnJumpReleased(const FInputActionValue& Value)
{
	bIsJumpPressed = false;
	bIsJumpJustPressed = false;
}

void ANeonDuskCharacter::OnFlyTriggered(const FInputActionValue& Value)
{
